...
def evaluate(model, test_images_path):
    ...
    test_results = model(test_images_path, save=True, save_txt=True, save_conf=True, imgsz=1280)

    test_csv = TEST_CSV_PATH  # Path to save test labels in CSV format.
...
def visualize_bboxes(image_id):
    ...
    with open(csv_path, "r") as csvfile:
        ...
        for row in csv_reader:
            # Check if image_id matches the current row's image_id
            if row[1] == image_id:
    ...
    # Display the image with bounding boxes using matplotlib for better Jupyter compatibility
    plt.imshow(img)
